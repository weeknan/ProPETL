...
class KittiTestVisualizer(Visualizer):
    ...
    def _draw_sem_seg(self, image: np.ndarray, sem_seg: PixelData,
        ...
        if len(sem_seg.shape) == 2:
            ...
            #sem_seg.shape is [H, W]
            #print(sem_seg[0][0])
            sem_seg = sem_seg.cpu().data
            #print(sem_seg[:, 0, 0])
            #print(sem_seg.shape)
            sem_seg = (255 * np.array(sem_seg)).astype('u1').squeeze()
            sem_seg = sem_seg[1]
            # print(sem_seg.shape)
            # print(sem_seg.sum())
            # print('------------')
            ...
            # for multi channel output, eg mask2former
            # sem_seg.shape is [2, H, W]
            #print(sem_seg.shape)
            sem_seg = sem_seg.cpu().data
            sem_seg = (255 * (np.array(sem_seg)[-1])).astype('u1').squeeze()
        # sem_seg = np.repeat(sem_seg, axis=0, repeats=3)
        # print(sem_seg.shape)
        # assert False
    ...
    def add_datasample(
        ...
        if (draw_pred and data_sample is not None
            ...
            pred_img_data = self._draw_sem_seg(pred_img_data,
                                               data_sample.seg_logits, # hear use the logits !, but not the real results
                                               classes, palette)
        ...
        if out_file is not None:
            ...
            #drawn_img.save(out_file)
            #print(drawn_img[:, 100, 500:550])
            temp = cv2.imwrite(out_file, drawn_img)
            #print(temp)
            # assert False
